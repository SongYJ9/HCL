...
if __name__ == "__main__":
    ...
    def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, args):
        ...
        def step(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, args):
            ...
            for (
                ...
                sys.stdout.write("\r")
                sys.stdout.write(
                    "%s: %.1f-%s-mix | Epoch [%3d/%3d], Iter[%3d/%3d]\t Labeled loss: %.4f, Unlabeled loss: %.4f"
                    % (
                        args.dataset,
                        args.r,
                        args.noise_mode,
        ...
        step(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, args)

        ### ADD: get hard label trainloader 
        # if epoch > args.hard_label_train_epochs:
        prob2, idx2 = eval_sort(net2)
        pred2 = np.zeros(50000)
        for i in range(50000):
            if i in idx2[:args.hard_label_num]:
                pred2[i] = 1

        hard_labeled_trainloader, hard_unlabeled_trainloader = loader.run(
            "train", pred2, prob2
        )  # co-divide

        print("\nHCL mix training:")
        step(epoch, net, net2, optimizer, hard_labeled_trainloader, hard_unlabeled_trainloader, args)
    ...
    def warmup(epoch, net, optimizer, dataloader):
        ...
        for batch_idx, (inputs, labels, path) in enumerate(dataloader):
            ...
            sys.stdout.write("\r")
            sys.stdout.write(
                "%s: %.1f-%s-mix | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f"
                % (
                    args.dataset,
                    args.r,
                    args.noise_mode,
