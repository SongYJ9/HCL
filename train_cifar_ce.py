...
if __name__ == "__main__":
    ...
    # Training
    ### CHANGE: add args
    def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, args):

        net.train()
        net2.eval()  # fix one network and train the other

        ### ADD: get hard label trainloader 
        if epoch > args.hard_label_train_epochs:
            idx2 = eval_sort(net2)
            pred2 = np.zeros(50000)
            for i in range(50000):
                if i in idx2[:args.hard_label_num]:
                    pred2[i] = 1
            hard_trainloader = loader.run(
                "hard", pred2, torch.zeros(50000)
            )  # co-divide
        ...
        for (
            ...
            ### ADD: hard label training and get loss
            if epoch > args.hard_label_train_epochs:
                try:
                    hard_inputs_x, hard_inputs_x2, hard_inputs_x3, hard_inputs_x4, hard_labels_x, _ = hard_train_iter.next()
                except:
                    hard_train_iter = iter(hard_trainloader)
                    hard_inputs_x, hard_inputs_x2, hard_inputs_x3, hard_inputs_x4, hard_labels_x, _ = hard_train_iter.next()
                
                hard_inputs = torch.cat([hard_inputs_x, hard_inputs_x2, hard_inputs_x3, hard_inputs_x4], dim=0).cuda()
                hard_labels_x = torch.cat([hard_labels_x, hard_labels_x, hard_labels_x, hard_labels_x], dim=0).cuda()
                
                hard_logits = net(hard_inputs)

                hard_Lx = F.cross_entropy(hard_logits, hard_labels_x, reduction='mean')
            else:
                hard_Lx = torch.tensor(0).cuda()
            ...
            ### CHANGE: add hard loss
            loss = Lx + lamb * Lu + penalty + hard_Lx
            ...
            ### CHANGE: add Hard labeled loss to show
            sys.stdout.write("\r")
            sys.stdout.write(
                "%s: %.1f-%s-ce | Epoch [%3d/%3d], Iter[%3d/%3d]\t Labeled loss: %.4f, Unlabeled loss: %.4f, CE loss: %.4f"
                % (
                    args.dataset,
                    args.r,
                    args.noise_mode,
    ...
    def warmup(epoch, net, optimizer, dataloader):
        ...
        for batch_idx, (inputs, labels, path) in enumerate(dataloader):
            ...
            sys.stdout.write("\r")
            sys.stdout.write(
                "%s: %.1f-%s-ce | Epoch [%3d/%3d] Iter[%3d/%3d]\t CE-loss: %.4f"
                % (
                    args.dataset,
                    args.r,
                    args.noise_mode,
    ...
    ### ADD: sort the output and use the highest confidence data 
    def eval_sort(model):
        model.eval()
        pre = torch.zeros(len(eval_loader.dataset))
        with torch.no_grad():
            for _, (inputs, targets, index) in enumerate(eval_loader):
                inputs = inputs.cuda()
                outputs = model(inputs)
                data, pred = torch.max(outputs, 1)
                data = data * (pred == targets.cuda())
                for b in range(inputs.size(0)):
                    pre[index[b]] = data[b]
        _, idx = torch.sort(pre, descending=True)
        return idx.numpy()
    ...
    while epoch < args.num_epochs:
        ...
        ### ADD
        if epoch >= args.lr_switch_epoch2: 
            lr /= 10
        ...
        if acc > maxacc:
            maxacc = acc
        print("Max Accuracy: ", round(maxacc*100, 3), "\n\n")
